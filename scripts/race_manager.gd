...
func _on_finish_line_crossed(body: Node, lap: int):
	...
	if progress.size() <= total_checkpoints:
		...
		if body == player_car:
			#print("Player completed lap:", current_lap)
		...
		pass
		#print(body.name, " crossed finish line but only passed ", progress.size(), "/", total_checkpoints, " checkpoints")

func _on_race_finished(car: Node):
	#print("Race finished for:", car.name)
	
	# Add to race position if not already there
	if not race_position.has(car):
		race_position.append(car)
		#print(car.name, " finished in position ", race_position.size())
